...
async function debugDeep() {
    ...
    // 2. Tab qua các element để tìm input thực sự
    console.log("Tabbing through elements...");
    for (let i = 0; i < 20; i++) {
        await page.keyboard.press('Tab');
        const active = await page.evaluate(() => {
            const el = document.activeElement;
            return { tag: el.tagName, text: el.innerText, html: el.outerHTML.substring(0, 100) };
        });
        console.log(`Tab ${i}: <${active.tag}> - ${active.text.substring(0, 20)}`);
        if (active.tag === 'DIV' && active.html.includes('contenteditable')) {
            console.log(">>> FOUND EDITABLE DIV VIA TAB!");
        }
    }
