...
async function tabVerification() {
    ...
    console.log("Starting Tab-by-Tab verification...");
    for (let i = 0; i <= 20; i++) {
        const active = await page.evaluate(() => {
            const el = document.activeElement;
            return el ? { tag: el.tagName, text: el.innerText, role: el.getAttribute('role') } : null;
        });
        console.log(`Tab ${i}: <${active?.tag}> ${active?.text || ''}`);
        await page.screenshot({ path: `tab_${i}.png` });
        
        // At Tab 15 (Input), try a forceful "Meta+A" and "Control+A"
        if (i === 15) {
            console.log("Attempting to clear input at Tab 15...");
            await page.keyboard.down('Control'); await page.keyboard.press('A'); await page.keyboard.up('Control');
            await page.keyboard.press('Backspace');
            await page.keyboard.type("PROMPT_VEO3_TEST");
            await page.screenshot({ path: `tab_15_typed.png` });
        }
        
        // At Tab 17 (Pill), try Enter
        if (i === 17) {
            console.log("Attempting to open menu at Tab 17...");
            await page.keyboard.press('Enter');
            await new Promise(r => setTimeout(r, 2000));
            await page.screenshot({ path: `tab_17_after_enter.png` });
        }
